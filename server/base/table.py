...
class ITable(object):
    ...
    def __init__(
        ...
        if index_field is not None:
            if not isinstance(index_field, list):
                raise TypeError(f"Index field must be list, but got {type(index_field)}")
    ...
    @lru_cache
    def get_indexed_records_map(self, index_field: Optional[list[str]]=None) -> dict[tuple, list[IRecord]]:
        index = index_field or self.index_field
        return group_by(self.__records, lambda record: record.index if index is not None else record.get_cell(index).raw_value)
...
class IBaseTable(ITable):
    ...
    def get_link_table(self, field_id: str) -> ITable:
        ...
        if field_id not in self.link_table:
            ...
            t = ILinkedBaseTable(
                ...
                field.linked_table_id,
                client,
                field.link_config.get("primary_key") if field.link_config is not None else None
            )
    ...
    def get_fields_from_field_map(
        ...
        if field_maps is not None:
            filtered_maps = list(filter(lambda m: m.get("source_field") is not None, field_maps))
