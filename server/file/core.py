...
class FileManager:
    ...
    def get_file_path_from_token_meta(self, token_meta: FileTokenMeta) -> str:
        ...
        return self.get_file_path(
            ...
            token_meta.base_id,
            token_meta.user_id,
            token_meta.created_time,
            token_meta.uuid,
            token_meta.filename
        )

    def get_file(self, token: str) -> FileItem:
    ...
    def save_file(
        ...
        if not self.can_save_file(
            tenant_key,
            user_id,
            base_id
        ):
            raise FileNumberLimitException(f"Each user is limited to upload {self.user_limit} files per base. ")
        if self.size_limit and len(data) > self.size_limit:
            raise InvalidateFileException(f"The size of file ({len(data) / (1024 * 1024)} MB) exceed file size limit ({self.size_limit / (1024 * 1024)} MB). ")
        uid = str(uuid.uuid4())
    ...
    def start_chunk(
        ...
        if self.size_limit and size > self.size_limit:
            raise InvalidateFileException(f"Exceed file size limit ({self.size_limit / (1024 * 1024)} MB). ")
        uid = str(uuid.uuid4())
        created_time = int(round(time() * 1000))
        file_token_meta = FileTokenMeta(
            tenant_key=tenant_key,
    ...
    def assemble_file_chunks(
        ...
        if target_md5 != chunk_meta.md5:
            ...
            raise InvalidateFileException(f"Assembled file's MD5({target_md5}) != upload file MD5({chunk_meta.md5}).")
    ...
    def get_chunk_meta(
        ...
        try:
          meta = read_json_file(os.path.join(file_dir, CHUNK_META_FILE_NAME))
            ...
            raise NoChunkMetaException("Not found chunk meta, please use chunk start API first.")
    ...
    def can_save_file(
        ...
        file_list = self.get_user_file_list(
            tenant_key,
            user_id,
            base_id
        )
    ...
    def get_user_file_list(
        ...
        glob_partern = os.path.join(base_dir, "*", "*", FILE_META_FILE_NAME)
        files = glob.glob(glob_partern)
