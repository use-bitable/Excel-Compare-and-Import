...
from server.request import ResponceStatusCode, make_responce
...
def AuthenticationMiddleware(
    ...
    def authenticate_request():
        ...
        if need_authentication(request.path):
          ...
          if not user_token:
              return make_responce(ResponceStatusCode.UNAUTHORIZED, msg="No user token provided"), 200
          user_token_security_key = request.headers.get(USER_TOKEN_SECURITY_KEY)
          if not user_token_security_key:
              return make_responce(ResponceStatusCode.UNAUTHORIZED, msg="No personal base token provided"), 200
          ...
          try:
            ...
            g.is_authenticated = True
              ...
              return make_responce(ResponceStatusCode.INVALIDATE_TOKEN, msg=f"Invalid user token or personalBaseToken: {e}"), 200
        else:
